...
3. **level 是目标节点的 # 数量**（# = 1, ## = 2, ### = 3...），必须准确
4. 一个请求可以包含多个操作
5. 如果用户的需求无法用上述操作实现，使用 replace 操作替换相关节点

## 层级限制

Markdown 标题最多 6 级（######）。当目标节点层级 ≥ 4 时，children/content 中的子节点层级不要超过 2 层深度，避免超出 6 级限制导致层级信息丢失。

## 示例

用户说"展开深度学习节点"，当前深度学习是 ## 级别：
\`\`\`json
[{"op":"expand","target":"深度学习","level":2,"children":"### 卷积神经网络\\n#### 图像分类\\n#### 目标检测\\n### 循环神经网络\\n#### 文本生成\\n#### 时序预测\\n### Transformer\\n#### 注意力机制\\n#### 自监督学习"}]
\`\`\`

用户说"删除计算机视觉"，当前计算机视觉是 ## 级别：
\`\`\`json
[{"op":"delete","target":"计算机视觉","level":2}]
\`\`\`

用户说"把机器学习改成ML"，当前机器学习是 ## 级别：
\`\`\`json
[{"op":"rename","target":"机器学习","level":2,"newName":"ML"}]
\`\`\`

用户说"在机器学习后面加一个迁移学习分支"，当前机器学习是 ## 级别：
\`\`\`json
[{"op":"insert","target":"机器学习","level":2,"content":"## 迁移学习\\n### 领域自适应\\n### 微调策略"}]
\`\`\``;
...
export default async function handler(req, res) {
  ...
  try {
    const { currentMarkdown, message, model, history, isFirstTurn } = req.body;
    ...
    // Token 优化策略:
    // - 首次对话 或 历史为空: 发完整 Markdown
    // - 后续对话且历史轮次 ≤ 6 条: 不发全文，依赖 history 上下文
    // - 后续对话但历史较多(> 6 条): 重新发全文同步状态，防止累积偏差
    const historyLen = Array.isArray(history) ? history.length : 0;
    const shouldSendFull = isFirstTurn || historyLen === 0 || historyLen > 6;
    ...
    if (shouldSendFull) {
      ...
      messages.push({
        role: 'user',
        content: `基于之前修改后的思维导图，新的修改要求：${message.trim()}\n\n请输出 JSON 操作指令数组。`,
      });
    }
